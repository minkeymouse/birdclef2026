#!/usr/bin/env python3
"""Patch notebook for v15: Perch 0.80 + SED29 0.10 + SED39 0.10 (EffNet diversity)."""
import json
import sys

NB = 'notebooks/birdclef-2026-perch-distill/notebook.ipynb'

# New SED block with per-ckpt backbone handling
NEW_BLOCK = """# --- 3-way z-score blend: Perch + SED29 + SED39 (exp39 EffNet-B0 NS-JFT) ---
SED29_CKPT_PATHS = [
...
SED39_CKPT_PATHS = [
    Path('/kaggle/input/datasets/ultimatumgame/birdclef2026-model-weights/exp39_effnet_sed.pt'),
    Path('/kaggle/input/birdclef2026-model-weights/exp39_effnet_sed.pt'),
]
SED29_CKPT = next((p for p in SED29_CKPT_PATHS if p.exists()), None)
SED39_CKPT = next((p for p in SED39_CKPT_PATHS if p.exists()), None)

# v15: v12 Perch weight (0.80) + split SED between HGNet + EffNet (diversity, train_audio only)
W_PERCH = 0.80
W_SED29 = 0.10
W_SED39 = 0.10
POST_BLEND_GAUSS_SIGMA = 0.5

if SED29_CKPT is not None or SED39_CKPT is not None:
    import torchaudio
    import timm
    ...
    if SED29_CKPT is not None:
        ...
        del _m; gc.collect()
    if SED39_CKPT is not None:
        print(f"\\nSED39 ckpt: {SED39_CKPT}")
        _m, _v, _bb = _load_sed(SED39_CKPT)
        print(f"  backbone={_bb}  val_auc={_v}")
        sed39_scores = _run_sed(_m, test_paths, desc='SED39')
        del _m; gc.collect()
    ...
    avail = {'perch': True,
             ...
             'sed39': sed39_scores is not None}
    raw_w = {'perch': W_PERCH, 'sed29': W_SED29, 'sed39': W_SED39}
    ...
    print(f"\\nBlend weights (renormalized): perch={w['perch']:.3f} "
          f"sed29={w['sed29']:.3f} sed39={w['sed39']:.3f}")
    ...
    if avail['sed39']:
        _sd39_z, _, _ = _zscore(sed39_scores[:, _blend_classes])
        _blend_z = _blend_z + w['sed39'] * _sd39_z
