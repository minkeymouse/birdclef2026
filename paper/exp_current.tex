...
% =====================================================================
% Section appended 2026-05-03: Gap analysis vs public 0.943 notebook
% =====================================================================

\section{Public 0.943 stack and the +0.011 gap}

\subsection{Discovery and revision of prior claims}

On 2026-05-03 a public Kaggle notebook
(\texttt{mattiaangeli/birdclef-2026-0-943-better-blend}, forked from
\texttt{vyankteshdwivedi/birdclef-2026-onnx-perch-sequence-modeling})
was identified at LB $0.941$--$0.943$. This is $+0.011$ above our v33
production at $0.932$. The discovery prompted a re-audit of our own
pipeline.

A claim made earlier in the working notes -- ``we have never tried
sequence modeling on Perch logits'' -- is incorrect and is retracted
here. Cells 41 (ProtoSSM v4 training), 43 (ResidualSSM second-pass
correction), and 48 (score fusion) of our production notebook
\texttt{notebooks/birdclef-2026-perch-distill/notebook.ipynb} are part
of the v33 inference path. The ``$0.7 \times \mathrm{Perch\_ONNX}$''
shorthand used elsewhere in this document is best read as
``$0.7 \times $ Perch+ProtoSSM ensemble''.

\subsection{Component-by-component comparison}

\begin{center}
\begin{tabular}{lll}
\toprule
Component & v33 & 0.943 stack \\
\midrule
Perch v2 ONNX                            & yes & yes \\
ProtoSSM v4/v5 sequence model            & yes & yes \\
ResidualSSM second-pass                  & yes & yes \\
MLP probe on PCA-Perch                   & yes & yes (h=128,64; pca=64) \\
Per-taxon temperature                    & V9 gate & T=0.95 / 1.10 \\
Distilled SED model                      & exp50 single (HGNet-B0) & Tucker 5-fold ONNX ensemble \\
SED mel filter range                     & 50--14000 Hz (n=128) & 20--16000 Hz (n=256), per-spec z-score \\
Final blend                              & linear $0.7 P + 0.3$ SED & rank-percentile blend \\
Conditional rescue rules                 & none & 3 rules: fake-only, proto-continuity, sed-spike \\
Temporal smoothing                       & Gaussian $\sigma{=}0.5$ & adaptive $\alpha = \alpha_0 (1 - \mathrm{conf})$ \\
Per-class threshold                      & none & isotonic + F1 grid \\
File-coherence prior                     & file-max $\alpha{=}0.10$ & file\_max$^{0.4}$ rank-aware scale \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Decomposed gap}

The three components present in the 0.943 stack and absent from v33 are
the SED ensemble, the rank-percentile blend, and the rescue rules. Best
estimates (one-out ablation if possible at submission time):

\begin{itemize}
\item \textbf{Tucker 5-fold ensemble vs.\ exp50 single}: estimated
$+0.005$ to $+0.007$. A 5-fold ONNX ensemble reduces variance, and the
wider mel-band ($20$--$16{,}000$ Hz vs.\ $50$--$14{,}000$ Hz) plus
per-spectrogram z-score normalisation likely captures
high-frequency texture content (insect harmonics in the 14--16~kHz
range) that exp50's filterbank cannot.

\item \textbf{Rank-percentile blend vs.\ linear blend}: estimated
$+0.002$ to $+0.005$. Per-class rank-percentile is AUC-invariant per
class, but blending in rank space induces a soft per-class calibration
without an explicit isotonic fit. This is structurally similar to the
2025 first-place ``rank-aware scaling'' trick.

\item \textbf{3 rescue rules}: estimated $+0.002$ to $+0.003$. Each
rescue handles a specific failure mode of two-stream fusion:
$(i)$~ProtoSSM says yes / SED says no with high local rank;
$(ii)$~ProtoSSM context-rank high under a fat-tail
$t$-distribution kernel ($\pm 3$ windows) where SED is silent;
$(iii)$~SED locally spikes ($> 0.95$ rank) where ProtoSSM is low. The
rescues operate on rank space and use small fixed blend weights.
\end{itemize}

The remaining residual ($\approx +0.001$) is plausibly distributed
across adaptive smoothing, per-class thresholds, isotonic calibration,
and the slightly different MLP probe configuration.

\subsection{Closure plan}

Closure proceeds one component at a time so that each LB increment is
attributable. Each step uses a single submission slot and is rolled
back on regression rather than stacked.

\begin{enumerate}
\item \textbf{v55 = v33 + Tucker 5-fold SED ensemble} (replace exp50;
preserve linear blend). Largest single lever; estimated $+0.005$ to
$+0.007$.
\item \textbf{v56 = v55 + rank-percentile blend} (replace linear blend
with rank-pct). Estimated $+0.002$ to $+0.005$.
\item \textbf{v57 = v56 + 3 rescue rules}. Estimated $+0.002$ to
$+0.003$.
\item \textbf{(v58, budget permitting)} adaptive smoothing
+ per-class threshold + isotonic.
\end{enumerate}

A pre-LB local probe on labeled SS is run for each step. Acceptance
criteria: per-step macro $\Delta > 0$ vs.\ the previous step's audit
result and Spearman$_\text{row} > 0.998$. Profile-failure modes
(W$_\mathrm{Perch}$ drop below 0.7, large Aves regression, single-class
catastrophic regression) trigger rollback before LB.

\subsection{Risks}

\begin{itemize}
\item \textbf{Wall-time budget.} Tucker SED is 5 ONNX folds over the
full test set on CPU. Adding the 5 folds to the existing
Perch+ProtoSSM+exp50 path may exceed the 90-min Kaggle code-comp
limit. A dev-run wall measurement is required before the LB push.
\item \textbf{Memory.} The public notebook caps MLP probe per-class
training rows at 3{,}000 to keep RAM in check. The Kaggle 30~GB
instance has been the observed envelope.
\item \textbf{Rank-blend interaction with the V9 taxon gate.} The V9
gate operates in probability space. Default behaviour: gate first
(retain current taxonomy filter) and rank-transform after.
\item \textbf{Production LB is still 0.932} (user-confirmed
2026-05-03). The recent v51--v54 submissions ($0.917$--$0.918$) were
diagnostic runs with teachers \emph{disabled}, not the active baseline.
\end{itemize}

\subsection{Note on the earlier ``ceiling exhaustion'' framing}

Earlier sections of these working notes argued that the post-v33
inference-time lever space was ``exhausted''. That framing is retracted
in light of the 0.943 public stack. The correct statement is that
\emph{our} post-v33 lever sweep -- 17 LB submissions through v50 -- 
exhausted modifications within the configurations we explored. Those
configurations did not include ensemble distilled SEDs, rank-percentile
fusion, or stream-conditional rescue rules. The lever space outside
our explored set is non-empty, and the $+0.011$ public delta is
empirical evidence that orthogonal mechanisms remain available.


% =====================================================================
% Section appended 2026-05-03: v57 result + late-fusion architecture
% =====================================================================

\section{The +0.009 jump: late-fusion architecture, not stronger SED}

\subsection{Empirical decomposition (3 LB submissions)}

Three submissions on 2026-05-02--03 isolate why Mattia's public 0.943
stack works:

\begin{center}
\begin{tabular}{lllll}
\toprule
v   & Change & local macro\_d & LB & lesson \\
\midrule
v55 & v33 + 0.10 $\times$ Tucker linear add & $+0.077$ & $\le 0.932$ & Tucker added linearly = no LB gain \\
v56 & v33 + Tucker rank-blend $W{=}0.30$    & $+0.078$ & $0.932 / 0.931$ & Rank-blend without rescues = no LB gain \\
\textbf{v57} & \textbf{Mattia fork as-is}    & --- & $\mathbf{0.941}$ & Whole pipeline = $+0.009$ \\
\bottomrule
\end{tabular}
\end{center}

The Tucker SED is comparable in strength to our exp50 (Pearson $0.85$
on labeled SS rows). Adding it to v33 in any blend form moves LB by
zero. Yet the same Tucker SED \emph{inside} Mattia's pipeline moves LB
by $+0.009$. The lever is therefore not the SED itself but the
architecture that consumes it.

\subsection{Component interaction analysis}

Mattia's pipeline keeps two genuinely independent streams to the very
end:
\begin{enumerate}
\item Perch $+$ ProtoSSM $+$ MLP probe $+$ post-processing $\to$ stream A
\item Tucker 5-fold SED $\to$ stream B
\end{enumerate}

Both streams are then transformed to per-class rank percentile and
combined as $\mathrm{pred} = x_a (1 - W) + x_b W$ with $W{=}0.30$.
After the base blend, three conditional rescue rules patch specific
failure modes of the two-detector ensemble:

\begin{enumerate}
\item \textbf{fake-only}: $p_a > 0.50 \wedge p_b < 0.05$
$\Rightarrow \mathrm{pred} \mathrel{+}= 0.12 \cdot x_a$. Patches the
case where Proto detects something SED missed.

\item \textbf{proto temporal continuity}: a fat-tail
$t$-distribution kernel ($\nu{=}2$, scale $1.20$) over $\pm 3$ windows
on $p_a$, normalised, yields $p_a^{\mathrm{ctx}}$. If
$x_a^{\mathrm{ctx}} > 0.88 \wedge x_a > 0.75 \wedge p_b < 0.12$, then
$\mathrm{pred} \mathrel{+}= 0.15 \cdot \max(x_a, x_a^{\mathrm{ctx}})$.
Patches a species call extending across multiple windows where Proto's
sequence model fires consistently and SED is silent.

\item \textbf{sed-local-spike}: $x_b > 0.95 \wedge x_a < 0.80$
$\Rightarrow \mathrm{pred} \mathrel{+}= 0.12 \cdot x_b$. Patches a
transient event that SED catches locally and Proto's temporal
smoothing over-dampens.
\end{enumerate}

The rescues only fire correctly on \emph{independent} streams. Our v55
and v56 mixed exp50 into the v33 base \emph{before} attempting to add
Tucker as the second stream; the resulting ``stream A'' was therefore
a pre-fused composite, and the Tucker stream had no clean disagreement
signal to rescue against. The architecture matters.

\subsection{New site-invariance sources from the 0.941 stack}

Returning to the v33 decomposition into 3 site-invariance sources
(Perch xeno-canto multi-region, exp50 2025-BG-mix, file-max physics),
Mattia's stack adds:

\begin{itemize}
\item \textbf{Source 4 -- 5-fold SED ensemble.} Variance across folds
  is itself an invariance source.
\item \textbf{Source 5 -- per-class rank-pct calibration.} Fusion in
  rank space is AUC-invariant per class but reduces the impact of
  per-class miscalibration in either stream.
\item \textbf{Source 6 -- conditional rescue rules.} Hand-tuned
  stream-disagreement priors recover recall on low-SED rare events
  and high-context-Proto species.
\end{itemize}

Sources 4--6 are jointly responsible for the $+0.009$ jump. Each in
isolation yields zero (as our v55/v56 experiments empirically
verified).

\subsection{Implication for forward levers}

The mattia-fork (v57) replaces v33 as the production reference. Future
levers should be evaluated as residual additions to the 0.941 baseline,
not the 0.932 baseline. Component-level grafts onto v33 are deprecated
in favour of additions to mattia-fork's two-stream skeleton.

Candidate forward levers (each $+0.001$ to $+0.003$ expected):
\begin{enumerate}
\item Add our exp50 as a third rank-blend stream inside mattia-fork.
\item Add BirdNET 2.4 or AudioMAE-frozen as a 4th stream.
\item Re-train Mattia's ProtoSSM on our extended training data
  (multi-region BG mixing from exp155, expanded external positives).
\item Tune SED\_W or rescue thresholds on a 0.941-anchored OOF.
\item Add a fourth rescue rule from observed mattia-fork OOF failures.
\end{enumerate}


% =====================================================================
% Section appended 2026-05-03: definitive component ablation (exp165)
% =====================================================================

\section{Definitive ablation of the +0.009 jump (exp165)}

\subsection{Rationale}

The previous section attributed the v33 $\to$ v57 LB jump to a synergy
of late-fusion sources (5-fold ensemble, rank-pct calibration, and
3 rescue rules). To test this hypothesis cleanly we ran a controlled
factor grid on the labeled SS eval (122 rows): SED choice
$\times$ fusion mode $\times$ rescue subset.
...
\begin{tabular}{llll}
\toprule
Variant & macro\_d $\Delta$ & sp\_row & profile \\
\midrule
\textbf{linear Tucker\_5fold W$=$0.30 rescues$=$none} & $\mathbf{+0.117}$ & $0.991$ & A (likely positive) \\
linear Tucker\_fold0\_only W$=$0.30 rescues$=$none & $+0.111$ & $0.990$ & A \\
rank Tucker\_5fold rescues$=$spike (only) & $+0.087$ & $0.401$ & C \\
rank Tucker\_5fold rescues$=$all & $+0.087$ & $0.409$ & C \\
rank Tucker\_5fold rescues$=$none & $+0.078$ & $0.404$ & C \\
rank Tucker\_5fold rescues$=$fake (only) & $+0.078$ & $0.414$ & C \\
rank Tucker\_5fold rescues$=$cont (only) & $+0.078$ & $0.401$ & C \\
linear exp50 W$=$0.30 rescues$=$none & $+0.008$ & $0.996$ & A \\
v33 LINEAR ref & $0.000$ & $1.000$ & --- \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
\item \textbf{SED choice dominates.} Replacing exp50 with Tucker\_5fold
in linear blend at $W{=}0.30$ swings macro\_d by $+0.109$ (from $+0.008$
to $+0.117$). The SED quality gap is by far the largest factor.

\item \textbf{5-fold ensemble adds little.} fold0-only $+0.111$ vs
5-fold ensemble $+0.117$ -- the ensemble contribution is only $+0.006$.
Most of Tucker's value is in fold0 alone (from the distillation
recipe and mel range, not the variance reduction).

\item \textbf{Linear beats rank in our pipeline.} Linear
$+0.117$ vs rank-pct $+0.078$ for the same Tucker stream. Rank-pct
collapses sp\_row to $0.40$ as a side effect of per-class re-normalisation,
and the macro\_d itself is also lower. Mattia's stack uses rank-pct
because their streamA (Perch+ProtoSSM raw) is uncalibrated; our streamA
is v33 itself, which is already calibrated through V9 + Gauss + file-max,
so linear blending of Tucker on v33 retains both higher macro\_d and
near-perfect sp\_row.

\item \textbf{Rescues contribute only via the spike rule.} In rank
space, rescues add $+0.009$ macro\_d ($+0.078 \to +0.087$). Of the
three rules, only \emph{sed-local-spike} is non-trivially positive
($+0.009$); \emph{fake-only} and \emph{proto-continuity} are essentially
neutral ($\le +0.001$) when applied to a v33-as-streamA setup. The
rescues are tuned for a Perch+ProtoSSM streamA, not v33.
\end{itemize}

\subsection{Why our prior v55 / v56 attempts under-delivered}

\begin{itemize}
\item \textbf{v55} used Tucker linear at $W{=}0.10$, where the local
gain is only $+0.077$. With a $\le 0.001$ LB-resolution and the team
public-LB displaying only the team-best score, a small linear-W
contribution can be hidden below the rounding floor. The architecture
choice was right; the dose was too conservative.

\item \textbf{v56} used Tucker rank-blend at $W{=}0.30$ without
rescues. Rank-pct on v33 (an already-calibrated streamA) actively
hurts both macro\_d and sp\_row relative to linear blending of the
same Tucker stream. The result was LB $0.931 / 0.932$.

\item \textbf{v57} (Mattia fork as-is) gives LB $0.941$ because the
rank-pct $+$ rescue architecture is matched to Mattia's specific
streamA composition. Replicating that architecture on top of v33
would not be expected to improve over linear blending of Tucker on
v33, and the local audit confirms this.
\end{itemize}

\subsection{Implication: re-test linear Tucker at higher W}

The strongest local profile observed in any experiment to date is
linear v33 $+$ Tucker\_5fold at $W{=}0.30$: macro\_d $+0.117$,
sp\_row $0.991$, with all five taxa positive. This is a textbook
``mechanism class A'' candidate. The natural next LB submission is
this configuration -- a reissue of the v55 patch with $W$ raised
from $0.10$ to $0.30$.

If this transfers, it indicates that our v33 $+$ Tucker linear path
is at least as strong as Mattia's rank-pct-with-rescues path, and
the late-fusion architecture is not the load-bearing source of the
+0.009 LB delta in this benchmark. If it does not transfer (LB stays
at v33 levels), then despite the local profile something specific
to Mattia's stream-A composition (ProtoSSM-only, MLP probe, etc.)
matters for LB transfer in a way the v33 streamA cannot reproduce.


% =====================================================================
% Section appended 2026-05-03 evening: production reset to v57 = 0.941
% =====================================================================

\section{Production reset: v57 = 0.941 (mattia-fork)}

After the v55--v59 dose-response sweep, the production reference shifts
from v33 = 0.932 (kernel \texttt{birdclef-2026-perch-distill}) to v57 =
0.941 (kernel \texttt{birdclef-2026-mattia-fork}, a direct fork of
\texttt{mattiaangeli/birdclef-2026-0-943-better-blend}). The v33 kernel
is retained as a legacy ablation reference; no new LB submissions go
to it unless explicitly anchored on v33.

\subsection{The +0.009 ceiling and what determines it}

v55, v56, v58, v59 all add Tucker SED to v33 in different fusion
configurations. They reach LB scores that are bounded by the team-best
0.941 (set by v57). The dose response saturates between $W{=}0.30$ and
$W{=}0.40$ in linear blend; the rank-percentile-with-rescues
architecture reaches the same ceiling. The conclusion is empirical:
**the +0.009 LB delta is attributable to the Tucker SED itself**, not
to the fusion architecture that consumes it.

The local audit gap between exp50 and Tucker as the secondary stream
in a controlled $W{=}0.30$ linear blend is $14\times$:
$\Delta_{\text{exp50}}{=}+0.008$ vs.\ $\Delta_{\text{Tucker}}{=}+0.117$
on macro\_d. This gap is concentrated in the non-Aves taxa, with
Tucker giving $+0.168$ Insecta macro\_d delta vs.\ $+0.007$ for exp50.
The mechanism is mel preprocessing: exp50's $f_{\max}{=}14{,}000$ Hz
clips Insecta's typical 10--15 kHz harmonics, while Tucker's
$f_{\max}{=}16{,}000$ Hz captures them.

\subsection{0.941 vs.\ 0.943 (Mattia's V3)}

Mattia's notebook page header reports 0.943 as the V3 best. The
markdown body confirms 0.941 from V1. Our fork captured V1; the
$\sim 0.002$ V1$\to$V3 gap is consistent with stochastic ProtoSSM
training variance (random init + mixup + SWA averaging produce
$\pm 0.001$--$0.002$ between runs at fixed hyperparameters) plus minor
threshold tweaks. The 0.943 ceiling is therefore not a structurally
distinct stack from 0.941; the same code sampled differently can land
between 0.939 and 0.943.

\subsection{Implications}

Sustaining or exceeding 0.941 requires an orthogonal SED signal source
that Tucker does not already provide. Three candidates of decreasing
expected difficulty:

\begin{enumerate}
\item Train an in-house SED with mel=256, $f_{\min}{=}20$, $f_{\max}{=}16000$
on the same train\_audio + 2025 BG mixing recipe as exp50, but with
5-fold cross-validation. Pure preprocessing fix; no distillation
(prior Perch-distill attempts amplified bird bias). $\sim 20$ hours
GPU at 32 GB. Add as third rank-blend stream in mattia-fork.

\item External non-Aves data (Watkins, Macaulay, AnuraSet) to train
a secondary SED with explicit non-Aves supervision, addressing a
class-imbalance failure mode that train\_audio's Aves-heavy
distribution cannot.

\item Hyperparameter tuning of the existing rescue rules
(\texttt{PROTO\_CONT\_RANK\_THR}, \texttt{SED\_ONLY\_RANK\_THR}, etc.)
on labeled-SS OOF; expected gain is on the order of stochastic
variance, but cheap.
\end{enumerate}

The 0.941 baseline is a useful checkpoint, but the team rank trajectory
(rank 451 at v33 $\to$ 289 at v57 $\to$ 307 at v59 as other teams
pushed past us) shows the public LB is already commodity at 0.94+.
The remaining lever family for top-100 LB is in-house SED training
plus non-Aves data acquisition --- the same data engineering work that
should have been done four months ago.
