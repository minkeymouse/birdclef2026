#!/usr/bin/env python3
"""exp84b Q4 — exp73 fine-tune scaled-up with expanded external pool.

Same pipeline as exp73 but uses ALL species directories under data/external/
(~30+ species after exp84 download), filters by exp50 confidence, and
fine-tunes longer (12 ep) with slightly higher LR for more data.

Compares val_SS (held-out 11-file) before/after.
"""
...
OUT = ROOT / "experiments" / "_data_pipelines" / "exp84b_q4_outputs"
...
BATCH_SIZE = 32; EPOCHS = 12; LR = 1e-4  # higher LR + more epochs for larger external pool
WD = 1e-2
