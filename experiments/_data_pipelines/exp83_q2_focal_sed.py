#!/usr/bin/env python3
"""exp83 Q2 — Same recipe as exp50 but with FOCAL LOSS instead of BCE.

Tests recipe-diversity hypothesis: does swapping the loss function (focal
γ=2) on the SAME data + architecture produce a model with sufficiently
different predictions to add value in ensemble blend with exp50?

Identical to exp50 in: data, HGNet-B0, mixup, 2025 BG, augmentation, LR.
Different from exp50: focal loss (γ=2) replaces BCE on both clip + fmax.

Output: experiments/exp83_q2_outputs/
"""
...
OUT = ROOT / "experiments" / "_data_pipelines" / "exp83_q2_outputs"
...
def focal_loss(logit, target, gamma=FOCAL_GAMMA, alpha=FOCAL_ALPHA):
    """Focal loss for multi-label binary case.
...
def hybrid_loss(clipwise, fmax, y, primary_idx, is_ta):
    """Same hybrid structure as exp50 but with focal loss substituted for BCE."""
    ta_mask = is_ta == 1; ml_mask = ~ta_mask
    losses = []
    if ta_mask.any():
        losses.append(focal_loss(clipwise[ta_mask], y[ta_mask]))
        losses.append(focal_loss(fmax[ta_mask], y[ta_mask]))
    if ml_mask.any():
        losses.append(focal_loss(clipwise[ml_mask], y[ml_mask]))
        losses.append(focal_loss(fmax[ml_mask], y[ml_mask]))
