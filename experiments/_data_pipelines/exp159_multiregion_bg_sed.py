#!/usr/bin/env python3
"""exp159 — Retrain SED with MULTI-REGION BG pool from train_audio quiet windows.

Motivation: exp47 mixup uses 2026 train_audio + labeled SS. Same-pool mixing
preserves site fingerprints → site-shortcut learning on rare single-site classes.
Solution: mix with RANDOM xeno-canto train_audio quiet windows from 12+
geographic buckets (multi-region) to force site-invariant feature learning
beyond what single-Pantanal BG (exp50) can provide.

Recipe same as exp47 (HGNet-B0 + 20s + BCE+BCE + fp32 + BN2d + SpecAug)
PLUS: with p=0.4 replace the mixup partner with a 2025 BG 5-sec window
(looped to 20s) blended at α~U(0.3,0.7). The 2025 BG has no labels — its
label stays at zeros, forcing target species to be discriminated against
completely different acoustic environments.

Output: experiments/exp159_outputs/
...
BG_PATH = ROOT / "experiments" / "_data_pipelines" / "exp155_outputs" / "bg_multiregion_raw.npz"
OUT = ROOT / "experiments" / "_data_pipelines" / "exp159_outputs"
...
BATCH_SIZE = 32; EPOCHS = 20; LR = 1e-3; WD = 1e-2
NUM_WORKERS = 4
MIXUP_ALPHA = 0.5; MIXUP_P = 0.6
BG_MIX_P = 0.4          # prob of using 2025 BG as mix partner
BG_ALPHA_LO, BG_ALPHA_HI = 0.1, 0.7  # widened low end (Soltero 2025: low-SNR robustness)
SECONDARY_WEIGHT = 0.3
WARMUP_EPOCHS = 2; EARLY_STOP_PATIENCE = 6
...
def bg_wav_to_20s(bg_5s):
    """Expand a 5-sec BG clip to 20s by tiling with slight offset randomness."""
    return np.tile(bg_5s, 4).astype(np.float32)[:CLIP_SAMPLES]


def raw_wave_mixup_with_bg(x, y, primary_idx, is_ta, bg_pool, alpha=MIXUP_ALPHA):
    """With BG_MIX_P probability, replace partner with 2025 BG clip (no label add).
       Otherwise standard same-batch mixup."""
    B = x.size(0)
    if bg_pool is not None and random.random() < BG_MIX_P:
        # pick B random 5-sec BG clips → 20s tiled
        idx = np.random.randint(0, bg_pool.shape[0], size=B)
        bg_wavs = np.stack([bg_wav_to_20s(bg_pool[i]) for i in idx])
        bg_t = torch.from_numpy(bg_wavs).to(x.device)
        lam = np.random.uniform(BG_ALPHA_LO, BG_ALPHA_HI)
        mixed_x = lam * x + (1 - lam) * bg_t
        mixed_y = y  # BG has no labels — keep original target labels
        mixed_primary = primary_idx
        mixed_isTA = is_ta
        ...
        mixed_primary = primary_idx.clone()
        mixed_isTA = torch.minimum(is_ta, is_ta[idx])
        return mixed_x, mixed_y, mixed_primary, mixed_isTA, lam
...
def train_epoch(model, loader, opt, dev, bg_pool=None):
    ...
    for x, y, pr, is_ta in loader:
        ...
        if random.random() < MIXUP_P:
            x, y, pr, is_ta, _ = raw_wave_mixup_with_bg(x, y, pr, is_ta, bg_pool)
...
@torch.no_grad()
def evaluate_ta(model, val_df, l2i, dev, batch=16, max_samples=2000):
    model.eval()
    n_cls = len(l2i)
    ...
    for i in range(0, n, batch):
        ...
        for k in range(i, j):
            ...
            wavs.append(center_crop(wav, CLIP_SAMPLES))
...
def main():
    ...
    print(f"Total train: TA {len(ta_train)} + SS {len(ss_train)}")
    print(f"Eval: TA {len(ta_val)}, SS {len(ss_eval)}")

    # Load 2025 BG pool
    bg_pool = None
    if BG_PATH.exists():
        bg_pool = np.load(BG_PATH)["windows"]
        print(f"Loaded 2025 BG pool: {bg_pool.shape}")
    else:
        print("WARNING: 2025 BG not found, using same-pool mixup only")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        ta_auc, n_ta = evaluate_ta(model, ta_val, l2i, DEVICE, max_samples=1500)
        ss_auc, n_ss = evaluate_ss(model, ss_eval, l2i, DEVICE)
        dt = time.time() - t0
        history.append({"epoch": ep, "lr": cur_lr, "loss": tr_loss,
                         "val_TA": ta_auc, "n_TA_cls": n_ta,
                         "val_SS": ss_auc, "n_SS_cls": n_ss,
        ...
        if ta_auc > best["val_TA"]:
            ...
            if patience >= EARLY_STOP_PATIENCE:
                print(f"Early stop at ep {ep}  best ep {best['epoch']} val_TA {best['val_TA']:.4f}", flush=True)
                break
