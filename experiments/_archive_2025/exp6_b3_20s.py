#!/usr/bin/env python3
"""
exp6_b3_20s.py — Larger backbone (EfficientNet-B3) + 20s training chunks.

Hypothesis: B0 is capacity-limited; 20s gives more temporal context for SED.
Changes from exp3:
  - Backbone: B0 (5.3M) → B3 (12M params)
  - Training duration: 10s → 20s
  - Batch size: 64 → 32 (memory)
  - Image size: 224 → 300 (B3 native)
"""
import os
import sys
import gc
...
CFG = {
    "seed": 42,
    "sr": 32000,
    "train_duration": 20.0,      # 20s chunks
    "infer_duration": 5.0,
    "n_fft": 2048,
    "hop_length": 512,
    "n_mels": 128,
    ...
    "weight_decay": 1e-2,
    "epochs": 15,
    "batch_size": 32,            # reduced for B3 + 300px
    "num_workers": 4,
    "n_folds": 5,
    "train_folds": [0, 1],
    "mixup_alpha": 0.4,
...
def precompute_train_audio(train_df):
    sr = CFG["sr"]
    target_len = int(sr * CFG["train_duration"])  # 20s = 640000 samples
    cache_sub = CACHE_DIR / "train_audio"
    cache_sub.mkdir(exist_ok=True)
    rows = []

    for idx, row in tqdm(train_df.iterrows(), total=len(train_df), desc="Precompute train_audio (20s)"):
        ...
        # Single chunk: pad/tile short files to 20s
        mel_id = f"ta_{idx}"
        mel_path = cache_sub / f"{mel_id}.npy"

        if not mel_path.exists():
            chunk = pad_or_crop(wav, target_len)
...
class MelDataset(Dataset):
    ...
    def _spec_augment(self, spec):
        ...
        if random.random() < 0.5:
            for _ in range(random.randint(1, 2)):
                mask_w = random.randint(5, min(40, w // 4))
...
def main():
    ...
    if meta_path.exists() and eval_meta_path.exists():
        ...
        print("Precomputing mel spectrograms (20s)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_meta, train_meta["primary_label"])):
        ...
        # Final soundscape eval with best
    ...
    with open(OUT_DIR / "exp6_results.json", "w") as f:
        json.dump(results, f, indent=2, default=lambda x: float(x) if isinstance(x, (np.floating,)) else x)
    with open(OUT_DIR / "exp6_config.json", "w") as f:
