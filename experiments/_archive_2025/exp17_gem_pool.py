#!/usr/bin/env python3
"""
exp17_gem_pool.py — GEM Frequency Pooling (1st place architecture).

1st place used "GEM frequency pooling" instead of attention pooling.
GEM (Generalized Mean) pooling is a learnable pooling that interpolates
between average (p=1) and max (p→∞) pooling via a learnable parameter p.

Also tests Cross-Entropy loss (1st place used CE, not BCE).

Base: exp14 recipe (5s chunks, mel-level augmentation, B0 backbone).
Changes: attention head → GEM + linear, BCE → CE where applicable.
"""
import os
import sys
import gc
...
class GeMFreqPool(nn.Module):
    """GEM pooling over frequency axis, then classify per-frame.
    1st place: 'Gem frequency pooling' — pools over frequency dimension
    while preserving temporal structure for SED."""
    ...
    def forward(self, x):
        """x: (B, C, F, T) → (B, C, T) via GeM over F dimension."""
        # Clamp to avoid numerical issues
        p = self.p.clamp(min=1.0)
        # GeM: (mean(x^p))^(1/p) over freq axis (dim=2)
        x = x.clamp(min=self.eps)
        pooled = (x.pow(p).mean(dim=2)).pow(1.0 / p)
        return pooled  # (B, C, T)


class BirdGeMModel(nn.Module):
    """SED model with GEM frequency pooling (1st place architecture)."""
    ...
    def __init__(self, backbone_name, num_classes, pretrained=True):
        ...
        self.gem_pool = GeMFreqPool(p_init=CFG["gem_p_init"], eps=CFG["gem_eps"])
    ...
    def forward(self, x):
        ...
        pooled = self.gem_pool(feat)  # (B, C, T)
        pooled = pooled.permute(0, 2, 1)  # (B, T, C)

        framewise = self.classifier(pooled)  # (B, T, num_classes)
        # Clipwise: max over time
        clipwise_max = framewise.max(dim=1).values
        # Clipwise: mean over time
        clipwise_avg = framewise.mean(dim=1)
        # Combine
        clipwise = 0.5 * clipwise_max + 0.5 * clipwise_avg
        return clipwise, clipwise_max
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_meta, train_meta["primary_label"])):
        ...
        model = BirdGeMModel(CFG["backbone"], NUM_CLASSES, pretrained=CFG["pretrained"]).to(DEVICE)
        ...
        for epoch in range(CFG["epochs"]):
            ...
            if ss_auc > best_ss:
                best_ss = ss_auc
                best_epoch = epoch + 1
                ckpt = WEIGHTS_DIR / f"exp17_gem_fold{fold}_best.pth"
