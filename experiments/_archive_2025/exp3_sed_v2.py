#!/usr/bin/env python3
"""
exp3_sed_v2.py — Improved SED: 10s input, attention head, mel-cache, improved augmentation.

Key changes from exp2:
  - 10s training chunks (more temporal context)
  - SED attention head (framewise max + clipwise dual supervision)
  - Spectrogram mixup (batch-level)
  - Labeled soundscapes held out for honest eval
  - Label deduplication
  - 2-fold training for ensemble
  - NS-JFT pretrained weights
"""
import os
import sys
import gc
...
OUT_DIR = ROOT / "experiments" / "exp3_outputs"
...
# ====================================================================
# Phase 1: Precompute mel spectrograms (10s chunks)
# ====================================================================
...
def precompute_train_audio(train_df):
    """Precompute 10s mel specs for train_audio."""
    import ast
    sr = CFG["sr"]
    ...
    for idx, row in tqdm(train_df.iterrows(), total=len(train_df), desc="Precompute train_audio"):
        ...
        # For 10s: extract up to 2 chunks
...
class AttentionHead(nn.Module):
    ...
    def forward(self, x):
        # x: (B, T, C)
        framewise = self.classifier(x)             # (B, T, num_classes)
        attn_weights = self.attention(x)           # (B, T, num_classes)
...
class BirdSEDModel(nn.Module):
    ...
    def forward(self, x):
        ...
        feat = feat.mean(dim=2).permute(0, 2, 1)  # pool freq, keep time: (B, T, C)
...
def main():
    ...
    if meta_path.exists() and eval_meta_path.exists():
        ...
        print("Phase 1: Precomputing mel spectrograms (10s)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_meta, train_meta["primary_label"])):
        ...
        state = torch.load(WEIGHTS_DIR / f"exp3_sed_fold{fold}_best.pth",
    ...
    with open(OUT_DIR / "exp3_config.json", "w") as f:
