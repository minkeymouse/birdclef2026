#!/usr/bin/env python3
"""
exp19_dedicated_taxa.py — Dedicated model for non-bird species.

1st place: "Insecta, Amphibia appears almost in all soundscapes"
"United target and additional species are 700, more epochs 40,
 larger batch size 128, smaller model efficientnet B0 for dedicated training."

72 non-bird species (Insecta=28, Amphibia=35, Mammalia=8, Reptilia=1) make up
30% of target species but only 2.1% of training data.

Approach:
  1. Train a dedicated B0 model on non-bird species only
  2. More epochs (30), oversample rare species aggressively
  3. Evaluate non-bird species AUC separately
  4. At inference: blend dedicated model's non-bird predictions with main model's
"""
import os
import sys
import gc
...
OUT_DIR = ROOT / "experiments" / "exp19_outputs"
...
CFG = {
    ...
    "lr": 1e-3,
    "weight_decay": 1e-2,
    "epochs": 30,  # 1st place: more epochs for dedicated model
    "batch_size": 128,  # 1st place: larger batch size
    "num_workers": 2,
    "n_folds": 5,
    "train_folds": [0, 1],
    "mixup_alpha": 0.4,
...
# ── Load taxonomy and identify non-bird species ──
...
NON_BIRD_CLASSES = ["Insecta", "Amphibia", "Mammalia", "Reptilia"]
non_bird_species = set(
    taxonomy_df[taxonomy_df["class_name"].isin(NON_BIRD_CLASSES)]["primary_label"].tolist()
)
NON_BIRD_INDICES = sorted([SPECIES2IDX[sp] for sp in non_bird_species if sp in SPECIES2IDX])
NUM_NON_BIRD = len(NON_BIRD_INDICES)

print(f"Total classes: {NUM_CLASSES}, Non-bird species: {NUM_NON_BIRD}")
print(f"Non-bird class breakdown:")
for cls in NON_BIRD_CLASSES:
    n = len(taxonomy_df[taxonomy_df["class_name"] == cls])
    print(f"  {cls}: {n} species")


# ====================================================================
# Dataset — trains on ALL species but focuses sampling on non-bird
# ====================================================================
...
def make_taxa_focused_sampler(df):
    """Heavily oversample non-bird species.
    Non-bird samples get 10x weight, effectively seeing them 10x more often."""
    is_non_bird = df["primary_label"].isin(non_bird_species)
    weights = np.where(is_non_bird, 10.0, 1.0)
...
# ====================================================================
# Model — full 234-class output but trained with non-bird focus
# ====================================================================
...
def validate(model, loader):
    """Returns overall AUC and non-bird-specific AUC."""
    model.eval()
    ...
    for c in range(NUM_CLASSES):
        ...
        if 0 < gt.sum() < len(gt):
            try:
                all_aucs.append(roc_auc_score(gt, all_preds[:, c]))
            except ValueError:
                pass

    # Non-bird AUC
    nb_aucs = []
    for c in NON_BIRD_INDICES:
        gt = (all_labels[:, c] > 0.5).astype(int)
        if 0 < gt.sum() < len(gt):
            try:
                nb_aucs.append(roc_auc_score(gt, all_preds[:, c]))
            except ValueError:
                pass

    overall = np.mean(all_aucs) if all_aucs else 0.0
    non_bird = np.mean(nb_aucs) if nb_aucs else 0.0
    return overall, non_bird, len(all_aucs), len(nb_aucs)
...
def main():
    ...
    # Count non-bird training samples
    n_non_bird = sum(1 for _, r in train_meta.iterrows() if r["primary_label"] in non_bird_species)
    print(f"Train: {len(train_meta)} ({n_non_bird} non-bird), Eval: {len(eval_meta)}, BG: {len(bg_mel_paths)}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_meta, train_meta["primary_label"])):
        ...
        for epoch in range(CFG["epochs"]):
            ...
            # Eval every 3 epochs + last
            if (epoch + 1) % 3 == 0 or epoch == CFG["epochs"] - 1 or epoch < 3:
                val_auc, val_nb, n_val, n_val_nb = validate(model, val_loader)
                ss_auc, ss_nb, ss_n, ss_nb_n = validate(model, eval_loader)

                print(f"E{epoch+1}/{CFG['epochs']} — loss: {train_loss:.4f}, "
                      f"val_auc: {val_auc:.4f}({n_val}), ss_auc: {ss_auc:.4f}({ss_n}), "
                      f"ss_nb_auc: {ss_nb:.4f}({ss_nb_n})")

                if ss_auc > best_ss:
                    best_ss = ss_auc
                    best_nb = ss_nb
                    best_epoch = epoch + 1
                    ckpt = WEIGHTS_DIR / f"exp19_taxa_fold{fold}_best.pth"
                    torch.save({"model_state_dict": model.state_dict(),
                                "epoch": epoch + 1, "ss_auc": ss_auc, "ss_nb_auc": ss_nb,
                                "val_auc": val_auc}, ckpt)
                    print(f"  -> Saved {ckpt.name} (ss_auc={ss_auc:.4f}, nb={ss_nb:.4f})")
            else:
                print(f"E{epoch+1}/{CFG['epochs']} — loss: {train_loss:.4f}")

        print(f"Fold {fold} best -> ss_auc: {best_ss:.4f}, nb_auc: {best_nb:.4f} @ E{best_epoch}")
        results[fold] = {"best_epoch": best_epoch, "ss_auc": best_ss, "ss_nb_auc": best_nb}
    ...
    print(f"\n{'='*60}\nRESULTS (exp19: Dedicated Non-Bird Taxa Model)")
    print(f"{'='*60}")
    exp14_ss = {0: 0.7820, 1: 0.7681}
    for fold, res in results.items():
        delta = res['ss_auc'] - exp14_ss.get(fold, 0)
        print(f"Fold {fold}: ss_auc={res['ss_auc']:.4f}, nb_auc={res['ss_nb_auc']:.4f}@E{res['best_epoch']}, Δ vs exp14: {delta:+.4f}")
    print(f"Time: {elapsed:.1f} min")

    with open(OUT_DIR / "exp19_results.json", "w") as f:
