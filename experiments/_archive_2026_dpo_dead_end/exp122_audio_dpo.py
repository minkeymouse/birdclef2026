#!/usr/bin/env python3
"""exp122 — End-to-end audio-level DPO from exp50 reference.

This is the experiment we should have done from the start.

Reference (π_ref): exp50 HGNet-B0 SED (val_SS 0.838, LB-positive in v24/v26).
Policy (π_θ): same architecture, init from exp50, trained with DPO loss
  end-to-end on raw audio. The BACKBONE updates, not just a head.

DPO loss (multi-label form):
  For each (preferred class p, rejected class q) pair sampled from row x:
    log π(c|x) = log_sigmoid(logit[c])  (per-class binary)
    margin = β * ((log π_θ(p) - log π_ref(p)) - (log π_θ(q) - log π_ref(q)))
    loss = -log_sigmoid(margin)

Hard-mining: each batch, score x with π_ref, find pairs where π_ref ranks
negative > positive (or within margin). Train DPO only on these.

Combined loss:
  L_total = (1-λ) * BCE_clipwise + λ * BCE_framemax + γ * DPO_hard

Augmentation: exp121-level cross-region (BG_MIX_P 0.5/0.85, 2025 BG).

Training: continue from exp50 ckpt, lr=2e-4 (low for fine-tune), 8 epochs.
~25-35 min/epoch end-to-end. Total ~4-5 hours.
"""
...
OUT = ROOT / "experiments/_data_pipelines/exp122_outputs"
...
BATCH_SIZE = 24  # smaller for memory (DPO needs both ref + policy forward)
EPOCHS = 8; LR = 2e-4; WD = 1e-2
NUM_WORKERS = 4
...
# DPO-specific
DPO_BETA = 1.0
DPO_LAMBDA = 0.5  # weight on DPO loss vs BCE
DPO_MARGIN_HARD = 0.5  # mine pairs where neg > pos - margin
DPO_MAX_PAIRS = 8  # max pairs per row
...
def build_ta_combined(l2i, val_frac=0.20, seed=SEED):
    ...
    if (DATA25 / "train.csv").exists():
        ...
        print(f"  TA combined: 2026={len(df_2026)}, 2025={len(df_2025)} (overlap={len(overlap)}), total={len(df_combined)}")
    else:
        df_combined = df_2026
...
def hard_mine_dpo_pairs(logits_ref, y, margin=DPO_MARGIN_HARD, max_per_row=DPO_MAX_PAIRS):
    """Mine (row_idx_in_batch, pos_class, neg_class) where ref ranks neg above pos."""
    ...
    for i in range(B):
        ...
        pos_l = logits_ref[i][pos_idx]
        neg_l = logits_ref[i][neg_idx]
        diffs = neg_l.unsqueeze(0) - pos_l.unsqueeze(1)  # (P, N)
        hard = (diffs > -margin).nonzero(as_tuple=False)  # (K, 2) with rows in pos_idx index
        if len(hard) == 0:
            continue
        # rank by violation
        violations = diffs[hard[:, 0], hard[:, 1]]
        top_k = torch.argsort(violations, descending=True)[:max_per_row]
        for k in top_k:
            p = int(pos_idx[hard[k, 0]])
            q = int(neg_idx[hard[k, 1]])
            triplets_b.append((i, p, q))
    return triplets_b


def dpo_loss_on_triplets(logits_policy, logits_ref, triplets, beta=DPO_BETA):
    """Compute DPO loss on pre-mined triplets."""
    if not triplets:
        return logits_policy.sum() * 0.0
    rows = torch.tensor([t[0] for t in triplets], device=logits_policy.device, dtype=torch.long)
    p_cls = torch.tensor([t[1] for t in triplets], device=logits_policy.device, dtype=torch.long)
    n_cls = torch.tensor([t[2] for t in triplets], device=logits_policy.device, dtype=torch.long)

    log_p_pos = F.logsigmoid(logits_policy[rows, p_cls])
    log_p_neg = F.logsigmoid(logits_policy[rows, n_cls])
    log_r_pos = F.logsigmoid(logits_ref[rows, p_cls])
    log_r_neg = F.logsigmoid(logits_ref[rows, n_cls])
...
def main():
    print("=== exp122: End-to-end audio DPO from exp50 ===\n", flush=True)
    set_seed(SEED)
    ...
    # Reference (frozen) and Policy (trainable) models
    print("\nLoading exp50 ckpt...")
    ref_model = SEDModel(n_cls).to(DEVICE)
    policy = SEDModel(n_cls).to(DEVICE)
    if EXP50_CKPT.exists():
        ...
        print(f"  Loaded exp50 ckpt as ref + policy init (val_TA={ckpt.get('val_auc', '?')})")
    else:
        raise FileNotFoundError("exp50 ckpt missing — required as π_ref")

    ref_model.eval()
    for p in ref_model.parameters():
        p.requires_grad_(False)

    optim = torch.optim.AdamW(policy.parameters(), lr=LR, weight_decay=WD)
    ...
    for ep in range(EPOCHS):
        policy.train()
        ep_bce = 0.0; ep_dpo = 0.0; ep_n_pairs = 0; n_batches = 0
        t0 = time.time()
        for batch in train_loader:
            ...
            # Ref forward (no grad)
            with torch.no_grad():
                clip_ref, _ = ref_model(x_m, train=False)

            # Hard-mine on ref logits
            triplets = hard_mine_dpo_pairs(clip_ref, y_m, margin=DPO_MARGIN_HARD,
                                              max_per_row=DPO_MAX_PAIRS)
            ep_n_pairs += len(triplets)

            # Policy forward (with grad)
            optim.zero_grad()
            clip_p, fmax_p = policy(x_m, train=True)

            # BCE
            loss_clip = F.binary_cross_entropy_with_logits(clip_p, y_m)
            loss_fmax = F.binary_cross_entropy_with_logits(fmax_p, y_m)
            bce_loss = 0.5 * loss_clip + 0.5 * loss_fmax

            # DPO loss
            dpo = dpo_loss_on_triplets(clip_p, clip_ref, triplets, beta=DPO_BETA)

            total = bce_loss + DPO_LAMBDA * dpo
            total.backward()
            optim.step()
            ep_bce += bce_loss.item(); ep_dpo += float(dpo); n_batches += 1
        scheduler.step()
        ...
        with torch.no_grad():
            for x, y, _, _ in val_loader_ta:
                x = x.to(DEVICE); clip, _ = policy(x, train=False)
        ...
        avg_pairs = ep_n_pairs / max(n_batches, 1)
        print(f"ep {ep:02d}  bce {ep_bce/n_batches:.4f}  dpo {ep_dpo/n_batches:.4f}  "
              f"avg_pairs/batch {avg_pairs:.1f}  "
              f"val_TA {val_ta:.4f} ({n_eval_ta})  val_SS {val_ss:.4f} ({n_eval_ss})  ({elapsed/60:.1f} min)", flush=True)
        ...
        if val_ss > best_val_ss:
            ...
            torch.save({"state_dict": best_state, "val_TA": val_ta, "val_SS": val_ss,
                          "epoch": ep, "config": {"backbone": BACKBONE,
                                                    "dpo_beta": DPO_BETA, "dpo_lambda": DPO_LAMBDA,
                                                    "seed": SEED}},
                         OUT / "best_ckpt.pt")
            print(f"  -> saved best ckpt @ ep{ep:02d}")
        history.append({"epoch": ep, "bce": ep_bce/n_batches, "dpo": ep_dpo/n_batches,
                          "val_TA": val_ta, "val_SS": val_ss, "avg_pairs": avg_pairs})
