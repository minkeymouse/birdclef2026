#!/usr/bin/env python3
"""exp159_score_eval — Score exp159 (multi-region BG SED) ckpt on labeled SS + v33 blend audit (replaces exp50)."""
...
DEVICE = "cuda"
EXP159_CKPT = ROOT / "experiments/_data_pipelines/exp159_outputs/best_ckpt.pt"
...
def get_exp159_scores(sc_g, ckpt_path):
    cache = EXP80 / "exp159_scores_labeled.npz"
    if cache.exists():
        return np.load(cache)["scores"]
    print("Running exp159 SED on labeled SS...")
...
def main():
    print("=== exp159b: Score exp159 + v33 blend audit ===\n", flush=True)
    ...
    exp159 = get_exp159_scores(sc_g, EXP159_CKPT)
    print(f"  exp159 scores: {exp159.shape}, range [{exp159.min():.5f}, {exp159.max():.5f}]")

    from scipy.stats import pearsonr
    print(f"\n  Pearson:")
    print(f"    Perch ↔ exp159: {pearsonr(perch_prob.flatten(), exp159.flatten())[0]:.3f}")
    print(f"    exp50 ↔ exp159: {pearsonr(exp50.flatten(), exp159.flatten())[0]:.3f}")
    print(f"    exp121 ↔ exp159: {pearsonr(exp121.flatten(), exp159.flatten())[0]:.3f}")

    ev_mask = sc_g.split.values == "eval"
    rows = [evaluate(v33, v33, ev_mask, Y, sp_taxon, "v33 ref")]

    # Replace exp50 with exp159 in v33 base
    base_new = 0.7 * perch_prob + 0.3 * exp159
    gated_new = apply_v9_gate(base_new, perch_emb, sp_taxon, offset=0.1)
    v33_new = file_max_blend(gated_new, sc_g, alpha=0.10)
    rows.append(evaluate(v33_new, v33, ev_mask, Y, sp_taxon, "v33-style: 0.7P + 0.3*exp159 (full swap)"))

    # Mix exp50 + exp159
    for w_123 in [0.10, 0.15, 0.20]:
        base_mix = 0.7 * perch_prob + (0.3 - w_123) * exp50 + w_123 * exp159
        gated_mix = apply_v9_gate(base_mix, perch_emb, sp_taxon, offset=0.1)
        v33_mix = file_max_blend(gated_mix, sc_g, alpha=0.10)
        rows.append(evaluate(v33_mix, v33, ev_mask, Y, sp_taxon, f"v33-style 0.7P+{0.3-w_123:.2f}exp50+{w_123}exp159"))

    # Additive on top of v33
    for w_123 in [0.05, 0.10, 0.15, 0.20]:
        P = (1 - w_123) * v33 + w_123 * exp159
        rows.append(evaluate(P, v33, ev_mask, Y, sp_taxon, f"v33 + {w_123}*exp159 additive"))
