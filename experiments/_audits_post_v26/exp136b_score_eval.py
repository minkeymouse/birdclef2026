#!/usr/bin/env python3
"""exp136bb — Score exp136b ckpt on labeled SS + v33 blend audit."""
...
DEVICE = "cuda"
EXP136B_CKPT = ROOT / "experiments/_data_pipelines/exp136b_outputs/best_ckpt.pt"
...
def get_exp136b_scores(sc_g, ckpt_path):
    cache = EXP80 / "exp136b_scores_labeled.npz"
    if cache.exists():
        return np.load(cache)["scores"]
    print("Running exp136b SED on labeled SS...")
...
def main():
    print("=== exp136bb: Score exp136b + v33 blend audit ===\n", flush=True)
    ...
    exp136b = get_exp136b_scores(sc_g, EXP136B_CKPT)
    print(f"  exp136b scores: {exp136b.shape}, range [{exp136b.min():.5f}, {exp136b.max():.5f}]")

    from scipy.stats import pearsonr
    print(f"\n  Pearson:")
    print(f"    Perch ↔ exp136b: {pearsonr(perch_prob.flatten(), exp136b.flatten())[0]:.3f}")
    print(f"    exp50 ↔ exp136b: {pearsonr(exp50.flatten(), exp136b.flatten())[0]:.3f}")
    print(f"    exp121 ↔ exp136b: {pearsonr(exp121.flatten(), exp136b.flatten())[0]:.3f}")

    ev_mask = sc_g.split.values == "eval"
    rows = [evaluate(v33, v33, ev_mask, Y, sp_taxon, "v33 ref")]

    # Replace exp50 with exp136b in v33 base
    base_new = 0.7 * perch_prob + 0.3 * exp136b
    gated_new = apply_v9_gate(base_new, perch_emb, sp_taxon, offset=0.1)
    v33_new = file_max_blend(gated_new, sc_g, alpha=0.10)
    rows.append(evaluate(v33_new, v33, ev_mask, Y, sp_taxon, "v33-style: 0.7P + 0.3*exp136b (full swap)"))

    # Mix exp50 + exp136b
    for w_123 in [0.10, 0.15, 0.20]:
        base_mix = 0.7 * perch_prob + (0.3 - w_123) * exp50 + w_123 * exp136b
        gated_mix = apply_v9_gate(base_mix, perch_emb, sp_taxon, offset=0.1)
        v33_mix = file_max_blend(gated_mix, sc_g, alpha=0.10)
        rows.append(evaluate(v33_mix, v33, ev_mask, Y, sp_taxon, f"v33-style 0.7P+{0.3-w_123:.2f}exp50+{w_123}exp136b"))

    # Additive on top of v33
    for w_123 in [0.05, 0.10, 0.15, 0.20]:
        P = (1 - w_123) * v33 + w_123 * exp136b
        rows.append(evaluate(P, v33, ev_mask, Y, sp_taxon, f"v33 + {w_123}*exp136b additive"))
