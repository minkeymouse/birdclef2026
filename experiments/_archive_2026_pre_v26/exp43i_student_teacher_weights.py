#!/usr/bin/env python3
"""exp43i — Student SED with pseudo-labels + 2025 1st prize recipe.

Recipe:
  Teacher: SED29 (sharpness-weighted, Mahal removed) (HGNet B0, train_audio only) — already done
...
Output:
  experiments/exp43i_outputs/best_ckpt.pt
  experiments/exp43i_outputs/val_scores.npz (Val-A 59 files)
"""
...
OUT = ROOT / "experiments" / "exp43i_outputs"
...
LR = 1e-3; WD = 1e-2
NUM_WORKERS = 8
MIXUP_P = 0.5   # 2025 1st key finding: always mix with pseudo
MIXUP_ALPHA = 0.5
DROP_PATH = 0.05  # stochastic depth
SS_TRAIN_SHARE = 0.20   # labeled SS train portion of batch
PSEUDO_SHARE = 0.15     # pseudo portion of batch
EARLY_STOP = 5
EVAL_N_FILES = 11
SEED = 42
...
def main():
    ...
    # Pseudo
    pseudo_df = pd.read_parquet(EXP41 / "pseudo_ensemble_df_teacher.parquet")
    pseudo_labels = np.load(EXP41 / "pseudo_ensemble_labels_teacher.npz")["labels"]
