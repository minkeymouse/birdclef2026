...
Output:
  - pseudo_train_df.parquet: (filename, start_sec, end_sec, soft_labels, weight)
"""
...
def main():
    d = np.load(EXP41 / "pseudo_probs.npz", allow_pickle=True)
    ...
    df.to_parquet(EXP41 / "pseudo_train_df.parquet")
    np.savez_compressed(EXP41 / "pseudo_soft_labels.npz", labels=soft_labels)
    print(f"\nSaved: {EXP41}/pseudo_train_df.parquet ({len(df)} rows)")
    print(f"Saved: {EXP41}/pseudo_soft_labels.npz")
