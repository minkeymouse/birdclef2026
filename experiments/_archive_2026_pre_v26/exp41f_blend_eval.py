...
EXP41 = ROOT / "experiments/exp41f_outputs"
...
def main():
    ...
    sed41f = inference(m, meta)
    np.savez_compressed(EXP41 / "val_scores_full.npz", preds=sed41f)
    ...
    S41f = sed41f[mask_eval]
    print(f"  Perch: {macro(Ye, Pe):.4f}")
    print(f"  SED29: {macro(Ye, S29e):.4f}")
    print(f"  SED41f: {macro(Ye, S41f):.4f}")

    zP, z29, z41 = zs(Pe), zs(S29e), zs(S41f)
    ...
    # Pearson SED29 vs SED41f
    from scipy.stats import pearsonr
    keep = Ye.sum(0) > 0
    r, _ = pearsonr(S29e[:, keep].flatten(), S41f[:, keep].flatten())
    print(f"\n  SED29 vs SED41f Pearson: {r:.3f}")

    # --- Full 59-file (leaked) for reference ---
    print("\n=== Full 59-file (leaked, upper bound) ===")
    print(f"  SED41f alone: {macro(Y, sed41f):.4f}")

    out = {
        "clean_11files": {
            "perch": macro(Ye, Pe),
            "sed29": macro(Ye, S29e),
            "sed41f": macro(Ye, S41f),
            "v12_ref": float(macro(Ye, v12)),
            "best_P_SED41f": {"alpha": float(best_2[1]), "val_a": float(best_2[0])},
            "best_3way": {"wP": float(best_3[1][0]), "w29": float(best_3[1][1]),
                          "w41": float(best_3[1][2]), "val_a": float(best_3[0])},
            "pearson_29_41": float(r),
        },
        "leaked_full": float(macro(Y, sed41f)),
    }
