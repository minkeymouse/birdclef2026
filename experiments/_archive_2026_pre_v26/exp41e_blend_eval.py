...
EXP41 = ROOT / "experiments/exp41e_outputs"
...
def main():
    ...
    sed41e = inference(m, meta)
    np.savez_compressed(EXP41 / "val_scores_full.npz", preds=sed41e)
    ...
    S41e = sed41e[mask_eval]
    print(f"  Perch: {macro(Ye, Pe):.4f}")
    print(f"  SED29: {macro(Ye, S29e):.4f}")
    print(f"  SED41e: {macro(Ye, S41e):.4f}")

    zP, z29, z41 = zs(Pe), zs(S29e), zs(S41e)
    ...
    # Pearson SED29 vs SED41e
    from scipy.stats import pearsonr
    keep = Ye.sum(0) > 0
    r, _ = pearsonr(S29e[:, keep].flatten(), S41e[:, keep].flatten())
    print(f"\n  SED29 vs SED41e Pearson: {r:.3f}")

    # --- Full 59-file (leaked) for reference ---
    print("\n=== Full 59-file (leaked, upper bound) ===")
    print(f"  SED41e alone: {macro(Y, sed41e):.4f}")

    out = {
        "clean_11files": {
            "perch": macro(Ye, Pe),
            "sed29": macro(Ye, S29e),
            "sed41e": macro(Ye, S41e),
            "v12_ref": float(macro(Ye, v12)),
            "best_P_SED41e": {"alpha": float(best_2[1]), "val_a": float(best_2[0])},
            "best_3way": {"wP": float(best_3[1][0]), "w29": float(best_3[1][1]),
                          "w41": float(best_3[1][2]), "val_a": float(best_3[0])},
            "pearson_29_41": float(r),
        },
        "leaked_full": float(macro(Y, sed41e)),
    }
